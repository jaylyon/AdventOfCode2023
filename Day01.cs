...
namespace AOC2023
{
    ...
    {
        private const string input = @"99lbqpxzzlbtvkmfrvrnmcxttseven
q7cnfslbtpkvseven
...
xcntwone4633sixmkm1nine";
        ...
        private static string Digitize(string s) { return (int.TryParse(s, out int result)) ? result.ToString() : Numbers[s]; }
        public int Part1() { return Solve(@"\d"); }
        public int Part2() { return Solve(@"\d|one|two|three|four|five|six|seven|eight|nine|zero"); }
        private int Solve(string pattern)
        {
            Regex re1 = new(pattern, options);
            Regex re2 = new(pattern, options | RegexOptions.RightToLeft);
            return lines.Select(l => int.Parse(string.Concat(Digitize(re1.Match(l).Value), Digitize(re2.Match(l).Value)))).Sum();
        }
    }
}
