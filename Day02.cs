...
namespace AOC2023
{
    ...
    {
        private const string input = @"Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
...
Game 100: 12 blue, 2 green; 2 green, 4 red, 13 blue; 9 blue, 1 red, 2 green; 2 green, 3 red, 1 blue";
        private readonly string[] lines = input.Split(Environment.NewLine);

        public int Part1()
        {
            var re = new Regex("Game (?<id>\\d*): ", RegexOptions.IgnoreCase | RegexOptions.Compiled);
            var reR = new Regex(@"(?<count>\d*) red", RegexOptions.IgnoreCase | RegexOptions.Compiled);
            var reG = new Regex(@"(?<count>\d*) green", RegexOptions.IgnoreCase | RegexOptions.Compiled);
            var reB = new Regex(@"(?<count>\d*) blue", RegexOptions.IgnoreCase | RegexOptions.Compiled);
            foreach (var line in lines)
            {
                var match = re.Match(line);
                var g = new Game() { Id = int.Parse(match.Groups["id"].Value) };
                foreach (var text in line.Replace($"Game {g.Id}: ", "").Split(';'))
                {
                    g.Plays.Add(new Play
                    {
                        Red = reR.Matches(text).Sum(m => int.Parse(m.Groups["count"].Value)),
                        Green = reG.Matches(text).Sum(m => int.Parse(m.Groups["count"].Value)),
                        Blue = reB.Matches(text).Sum(m => int.Parse(m.Groups["count"].Value))
                    });
                }
                _games.Add(g);
            }
            return _games.Where(g => g.Plays.TrueForAll(p => p is { Red: <= 12, Green: <= 13, Blue: <= 14 })).Sum(g => g.Id);
        }

        public int Part2()
        {
            return _games.Sum(g => g.Power);
        }
        ...
        private record Play
        {
            internal int Red { get; init; }
            internal int Green { get; init; }
            internal int Blue { get; init; }
        }
    }
}
